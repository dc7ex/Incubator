...
func CheckBirthdayValid(nYear, nMonth, nDay int) bool {
	...
	if 2 == nMonth {
		if IsLeapYear(nYear) && nDay > 29 {
			return false
		} else if !IsLeapYear(nYear) && nDay > 28 {
			return false
		}
...
func GetCitizenNoInfo(citizenNo []byte) (err error, birthday, sex, address string) {
	...
	birthday = string(citizenNo[6:10]) + "-" + string(citizenNo[10:12]) + "-" + string(citizenNo[12:14])
...
func VerifyName(name string) bool {
	//	regular := "^[\\x{4e00}-\\x{9fa5}]{2,10}$"
	regular := `^[\x{4e00}-\x{9fa5}]+(·[\x{4e00}-\x{9fa5}]+)*$`
